...
async function getVideoSources(ctx, id, media) {
  ...
  const data = await ctx.fetcher(path, {
    baseUrl: "https://lmscript.xyz",
    query: { expand: "streams", id }
  });
  return data;
}
async function getVideoUrl(ctx, id, media) {
...
async function scrape(ctx, media, result) {
  ...
  if (media.type === "movie") {
    ...
    const data = await ctx.fetcher(`/v1/shows`, {
      baseUrl: "https://lmscript.xyz",
...
const superStreamScraper = makeSourcerer({
  ...
  async scrapeShow(ctx) {
    ...
    if (fid === void 0)
      throw new NotFoundError("No streamable file found");
    return {
      embeds: [],
      stream: {
